...
#pragma omp parallel for num_threads(2)
        for (size_t ori = 0; ori < 2; ++ori)
        {
            ...
            std::unordered_map<uint32_t, std::set<std::tuple<size_t, size_t, size_t>, decltype(custom_comparator)>> id_to_frame_block_id_and_seq_pos;

            for (size_t frame = 0; frame < 3; ++frame)
            {

                btllib::AAHash aahash(sixframed_xlated_proteins[frame + ori * 3], hash_num, kmer_size, 1);
                aahash.roll();
                ...
                while (aahash.get_pos() != std::numeric_limits<size_t>::max())
                {
                    ...
                    size_t seq_pos = aahash.get_pos() - 9;
                    ...
                    while (extend_block && aahash.get_pos() != std::numeric_limits<size_t>::max())
                    {
                        ...
                        {
                            auto temp_ID_pos = mi_bf.get_id(aahash.hashes());
                            // check if all IDs in id_set are in temp_ID_pos and set found to true if all are found
                            bool found = false;
                            for (auto &ID : id_set)
                            {
                                found = false;
                            ...
                            if (found)
                            {
                                ...
                                for (size_t i = 0; i < ids_vec.size(); ++i)
                                {
                                    ...
                                    for (auto &pos : temp_pos_set)
                                    {
                                        if (pos == *id_to_pos_set[ids_vec[i]].end() + 1)
                                        {
                                            // std::cerr << "pos: " << pos << std::endl;
                    ...
                    for (auto &ID_pos_set : id_to_pos_set)
                    {
                        frame_to_block_id_to_id_and_pos[frame][block_id] = std::make_pair(*ID_pos_set.second.begin(), *ID_pos_set.second.end());
...
#pragma omp critical
            ...
            for (auto &ID_count : id_to_count_across_all_frames)
            {
                ...
                {
                    size_t frame = std::get<0>(frame_block_id_and_seq_pos);
                    ...
                    if (frame_to_block_id_to_id_and_pos[frame].find(block_id) != frame_to_block_id_to_id_and_pos[frame].end())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                end_pos = frame_to_block_id_to_id_and_pos[frame][block_id].second;
                                if (end_pos + 1 == frame_to_block_id_to_id_and_pos[frame][block_id].first)
                                {
                                    adjusted_kmer_counts += block_len;
                                }
                                else
                            ...
                            else
                            {
                                ...
                                else if (adjusted_kmer_counts > 0.5 * expected_kmer_counts)
                                {
                                    incomplete_copies++;
                                }
                                end_pos = frame_to_block_id_to_id_and_pos[frame][block_id].second;
