...
int main(int argc, char **argv)
{
    ...
    btllib::MIBloomFilter<uint64_t> mi_bf(calc_optimal_size(std::max<size_t>(genome_size, 1000000), hash_num, 0.1), hash_num);
...
#pragma omp parallel
    ...
    std::ofstream gff_output_file(output_prefix + ".gff");
    gff_output_file << "##gff-version 3" << std::endl;
    ...
    std::set<std::tuple<std::string, size_t, size_t, double, std::string, std::string>, decltype(gff_comparator)> gff_set(gff_comparator);
